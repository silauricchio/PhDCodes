...
#ifndef Reweight2D_1map_h
#define Reweight2D_1map_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Reweight2D_1map(TTree *tree=0, TString tree_path="",  TString tree_name="", TString path_histo="", TString histo_name="", TString var_nameX="", TString var_nameY="");
   virtual ~Reweight2D_1map();
   ...
   void  Set_histo_name(TString histo_name)
   {
     name_histo_ =  histo_name;
   }
  
   void     Set_var_nameX(TString var_nameX){var_nameX_ = var_nameX;}
   void     Set_var_nameY(TString var_nameY){var_nameY_ = var_nameY;}
...
#ifdef Reweight2D_1map_cxx
Reweight2D_1map::Reweight2D_1map(TTree *tree, TString tree_path="",  TString tree_name="", TString path_histo="", TString histo_name="", TString var_nameX="", TString var_nameY="") : fChain(0) 
...
Init(tree);
 ...
 Set_histo_name(histo_name);
 Set_var_nameX(var_nameX);
 Set_var_nameY(var_nameY);
 Set_tree_name(tree_name);
 
 Loop();
}

Reweight2D_1map::~Reweight2D_1map()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Reweight2D_1map::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Reweight2D_1map::LoadTree(Long64_t entry)
{
...
void Reweight2D_1map::Init(TTree *tree)
{
...
void Reweight2D_1map::Show(Long64_t entry)
{
...
Int_t Reweight2D_1map::Cut(Long64_t entry)
{
...
#endif // #ifdef Reweight2D_1map_cxx
